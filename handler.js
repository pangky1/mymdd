        ...
        try {
            ...
            try {
                ...
                if (user) {
                    ...
                    if (!('warnl' in user)) user.warnl = false
                    if (!isNumber(user.warning)) user.warning = 0
                    if (!isNumber(user.lastIstigfar)) user.lastIstigfar = 0
                    if (!isNumber(user.call)) user.call = 0  
                    if (!isNumber(user.bio)) user.bio = ''

                    if (!isNumber(user.afk)) user.afk = -1
                    if (!('afkl' in user)) user.afkl = false
            ...
            for (let name in global.plugins) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                        this.sendButton(m.chat, `Limit anda habis, silahkan beli melalui *${usedPrefix}buy*, ingin limit unlimited? hubungi owner/klik button di bawah`, wm, 'Owner', '.owner', m)
                ...
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        ...
                        try {
                            ...
                            await this.sendButtonImg(id, action === 'add' ? wel : lea, text, wm, action === 'add' ? 'selamat datang' : 'sampai jumpa', action === 'add' ? '.intro' : 'FokusID', fkontak)
                        }
                    }
                }
                break
