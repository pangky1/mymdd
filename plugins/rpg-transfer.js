            ...
            case 'limit':
                if (global.db.data.users[m.sender].limit >= count * 1) {
                    try {
                        ...
                        conn.sendButton(m.chat, `Berhasil mentransfer ${count} ${rpg.emoticon('limit')}Limit`.trim(), wm, 'inventory', usedPrefix + 'inv', m)
                    } catch (e) {
                        global.db.data.users[m.sender].limit += count * 1
                        m.reply('Gagal Menstransfer')
                        console.log(e)
                    }
                } else conn.sendButton(m.chat, `${rpg.emoticon('limit')}Sampah kamu tidak cukup`.trim(), wm, 'inventory', usedPrefix + 'inv', m)
                break
            case 'diamond':
